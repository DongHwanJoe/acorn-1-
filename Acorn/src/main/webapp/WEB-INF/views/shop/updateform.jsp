				...
				<div class="row">
					...
					<div class="col-8" style="text-align: left; padding-left: 20px;">
						<select class="dropdown" name="categorie" id="categorie">
							<option value="선택"
								<c:if test="${dto.categorie eq '선택'}">selected</c:if>>선택</option>
							<option value="한식"
								<c:if test="${dto.categorie eq '한식'}">selected</c:if>>한식</option>
							<option value="분식"
								<c:if test="${dto.categorie eq '분식'}">selected</c:if>>분식</option>
							<option value="일식"
								<c:if test="${dto.categorie eq '일식'}">selected</c:if>>일식</option>
							<option value="양식"
								<c:if test="${dto.categorie eq '양식'}">selected</c:if>>양식</option>
							<option value="중식"
								<c:if test="${dto.categorie eq '중식'}">selected</c:if>>중식</option>
							<option value="패스트푸드"
								<c:if test="${dto.categorie eq '패스트푸드'}">selected</c:if>>패스트푸드</option>
							<option value="기타"
								<c:if test="${dto.categorie eq '기타'}">selected</c:if>>기타</option>
						</select>
					</div>
				</div>
				<br />
	...
	<script>
		...
		document.querySelector("#image").addEventListener("change",function() {
			...
			.then(function(data) {
				...
				// img 요소를 문자열로 작성한 다음 
				let img = `<img id="profileImage" src="${pageContext.request.contextPath }/shop/images/\${data.imagePath}">`;
